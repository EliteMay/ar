// ASMRTube timestamp parser v1.9
// Multi-stage parser for diverse YouTube timestamp/comment formats.
(function(){
  ...
  function timestampTags(label,group='',subtitle=''){
    const source=`${group} ${label} ${subtitle}`.trim();
    const tags=[];
  ...
  function findVisibleTimestampTokens(source){
    ...
    while((m=re.exec(source))){
      const timeText=m[1]||m[4];
      const seconds=parseFlexibleTime(timeText);
      if(seconds==null)continue;
      const rawStart=m.index;
      const before=source[rawStart-1]||'';
  ...
  function splitSegment(segment){
    const s=markdownToText(segment).replace(/\r/g,'');
  ...
  function makeEntries(source,tokens){
    return tokens.map((token,i)=>{
      ...
      const cleaned=cleanText(pieces[0]||'')||'タイムスタンプ';
      const bi=splitBilingual(cleaned);
  ...
  window.parseTimestampText=function(text){
    ...
    const entries=makeEntries(source,tokens);
    const out=[];
    let section='';
    let sectionKind='';
    let activeParent=null;
    let lastNonChild=null;
    ...
    for(let i=0;i<entries.length;i++){
      ...
      if(sectionKind==='side'&&!isChildLabel(entry.label)&&entry.markerType==='none'){
        section='';sectionKind='';activeParent=null;
      }

      if(entry.markerType==='parent'){
        role='parent';group=sectionKind==='explicit'?section:'';
      ...
      }else if(sectionKind==='side'&&section&&isChildLabel(entry.label)){
        role='child';group=section;inferred=true;
      }else if(sectionKind==='explicit'&&section){
        group=section;
      }
      ...
      const row={
        time:entry.time,
        label:entry.label,
        tags:timestampTags(entry.label,group,entry.subtitle),
        group,
        role,
        depth:role==='child'&&parentTime!=null?1:0,
        confidence,
      ...
      if(entry.candidate&&next){
        if(next.markerType==='parent'||next.markerType==='child'){
          section=entry.candidate;sectionKind='explicit';activeParent=null;
        }else if(next.markerType==='none'&&isChildLabel(next.label)){
          section=entry.candidate;sectionKind='side';activeParent=null;
        }
  ...
  window.getTimestampParseStats=function(){return {...lastStats}};
  window.guessTags=function(label,group='',subtitle=''){return timestampTags(String(label||''),String(group||''),String(subtitle||''))};
  window.__asmrTimestampParser={parseTime:parseFlexibleTime,splitBilingual};
})();
