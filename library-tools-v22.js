...
(function(){
  ...
  const getItem=()=>{try{return typeof itemById==='function'?itemById(state.selectedId):null}catch{return null}};
  ...
  // Search section names as well as the v2.1 title/tag/timestamp search.
  const previousFiltered=filtered;
  filtered=function(){
    const query=state.query;
    if(!query)return previousFiltered();
    state.query='';let items=[];
    ...
    return items.filter(item=>{
      ...
      const sections=(item.favoriteSections||[]).flatMap(x=>[x.label,fmtTime(x.start),fmtTime(x.end)]);
      const haystack=normalize([item.title,item.creator,...(item.tags||[]),...timestamps,...sections].filter(Boolean).join(' '));
  ...
  function createSectionDialog(){
    ...
    $('#favoriteSectionForm').addEventListener('submit',event=>{
      ...
      sectionsOf(item).push({id:makeId(),label,start,end,createdAt:Date.now()});item.favoriteSections.sort((a,b)=>a.start-b.start);save();dialog.close();enhanceInfoCard(true);toast('お気に入り区間を保存しました');
    });
  }
  ...
  function openSectionDialog(){
    ...
    $('#favoriteSectionLabel').value='';$('#favoriteSectionStart').value=fmtTime(start);$('#favoriteSectionEnd').value=fmtTime(end);$('#favoriteSectionDialog').showModal();setTimeout(()=>$('#favoriteSectionLabel')?.focus(),0);
  }
  ...
  function showCreatorPage(creator){
    const name=String(creator||'').trim();if(!name)return toast('配信者が設定されていません');createCreatorDialog();
    ...
    const tagMap=new Map();items.forEach(x=>(x.tags||[]).forEach(tag=>tagMap.set(tag,(tagMap.get(tag)||0)+1)));const tags=[...tagMap.entries()].sort((a,b)=>b[1]-a[1]).slice(0,10);
    ...
    const dialog=$('#creatorDialog');$$('[data-creator-item]',$('#creatorDialogBody')).forEach(button=>button.onclick=()=>{dialog.close();window.asmrtubeProductShell?.hideDashboard?.();selectItem(button.dataset.creatorItem)});dialog.showModal();
  }
  ...
  function enhanceInfoCard(force=false){
    ...
    const sections=sectionsOf(item),resume=Number(item.resumeAt||0),resumeDuration=Number(item.resumeDuration||0),wrapper=document.createElement('div');wrapper.className='v22-library-tools';wrapper.dataset.signature=sig;
    ...
    $('#resumePlayBtn',wrapper)?.addEventListener('click',()=>playItem(item.id,resume));$('#creatorPageBtn',wrapper)?.addEventListener('click',()=>showCreatorPage(item.creator));$('#saveFavoriteSectionBtn',wrapper)?.addEventListener('click',openSectionDialog);
  ...
  function saveTimestampEdits(event){
    ...
    const parentMap=new Map();drafts.forEach(row=>{if(row.data.role==='parent'&&row.originalTime!=null)parentMap.set(Number(row.originalTime),{time:Number(row.data.time),label:row.data.label})});
    drafts.forEach(row=>{const data=row.data;if(data.role!=='child'||data.parentTime==null)return;const parent=parentMap.get(Number(data.parentTime));if(parent){data.parentTime=parent.time;data.parentLabel=parent.label;data.depth=1}else{data.role='item';data.depth=0;delete data.parentTime;delete data.parentLabel}});
