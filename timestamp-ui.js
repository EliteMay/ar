...
(function(){
  ...
  window.renderTimestamps=function(){
    const x=window.state?.library?.find?.(v=>v.id===window.state.selectedId) || (typeof itemById==='function'?itemById(state.selectedId):null);
    ...
    let lastGroup='__initial__';
    let html='';
    rows.forEach((t,i)=>{
      const group=String(t.group||'').trim();
      if(group&&group!==lastGroup){
        html+=`<div class="timestamp-group">${esc(group)}</div>`;
      }
      lastGroup=group;
  ...
  function saveGroupedTimestamps(){
    ...
    for(const t of state.parsedTimestamps){
      const group=String(t.group||'').trim();
      if(!x.timestamps.some(z=>z.time===t.time&&z.label===t.label&&String(z.group||'')===group)){
        x.timestamps.push({...t,group});
      }
    }
